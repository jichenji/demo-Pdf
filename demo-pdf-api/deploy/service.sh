...
#java8 remove -XX:PermSize=128m -XX:MaxPermSize=320m
#C server: "-server -Xms4g -Xmx4g -Xmn1500m "
#B server: "-server -Xms2g -Xmx2g -Xmn750m "
JAVA_OPT_1="-server -Xms4g -Xmx4g -Xmn1500m "
...
JAVA_OPTS="${JAVA_OPT_1} ${JAVA_OPT_2} ${JAVA_OPT_4} ${JAVA_OPT_5} ${JAVA_OPT_6}"
...
function checkProcessIsRunning {
   local pid="$1"
   ps -ef | grep java | grep $pid | grep "$SERVICE_NAME" | grep -q --binary -F java
...
function startServiceProcess {
   touch $PID_FILE
   #rm -rf nohup.log
   nohup $JAVA -jar $SERVICE_NAME $JAVA_OPTS >> nohup.log 2>&1 & echo $! > $PID_FILE
   sleep 0.1
   pid="$(<$PID_FILE)"
